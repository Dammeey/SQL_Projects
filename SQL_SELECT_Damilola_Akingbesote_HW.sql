...
SELECT  a.address || '' || '' as rental_store, sum(amount) as revenue -- selecting the appropriate columns and also adding up the amounts column
...
WITH staff_bonus AS 
(SELECT s.store_id, s.first_name || ' ' || s.last_name as staff_member, SUM(p.amount) as revenue_made
FROM public.staff s
...
SELECT DISTINCT ON (store_id)
                store_id, staff_member, revenue_made
FROM staff_bonus
